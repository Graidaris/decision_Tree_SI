...
for line in arr:
	features.append([])
	for number in line:
		features[n_line].append(float(number))
	n_line = n_line + 1;	
features.pop()
...
l = open("ne_labels.data")

labels = [int(item) for item in l.readline().rstrip().split(" ")]
...
featuresNames = ["age", "sex", "cp", "trestbps","chol", "fbs", "restecg", "exang", "thalach", "oldpeak", "slope", "ca", "thal"]
classNames = ["zdrowy", "niezdrowy", "niezdrowy", "niezdrowy", "niezdrowy"]

featuresNamesPolish = ["wiek", "płeć", "rodzaj bólu w klatce", "ciśnienie krwi w spoczynku", "cholesterol", "poziom cukru", "wyniki EKG", "duszność", "tętno max", "obniżenie ST", "nachylenie ST", "ca", "talasemia"]
...
def print_tree(t, root=0, depth=1):
    ...
    if left_child == sklearn.tree._tree.TREE_LEAF:
        ...
        print(indent + 'if X_i' + str(t.feature[root]) + ' < %.2f: # (node %d)' % ( t.threshold[root], root))
